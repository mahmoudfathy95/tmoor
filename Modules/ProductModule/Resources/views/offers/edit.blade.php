...
@section('content')
        ...
        <div class="col-md-12">
            ...
            <div class="card card-primary">
                ...
                <!-- /.card-header -->
                <!-- form start -->
                <form id="create1" role="form" action="{{route('offers.update',$item->id)}}" method="post" enctype="multipart/form-data">
                    @csrf
                    @method('put')
                    <div class="card-body">
                    <div class="row">
                     <div class="form-group col-md-6">
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <select name="products[]" class="form-control select2" data-placeholder="Choose Browser" multiple="multiple">
                                ...
                                <option {{in_array($key->id,$arr)?'selected':''}} value="{{$key->id}}">{{$key->name}}</option>
                                @endforeach
                            </select>
                            @error('category_id')
                                <div style="background:transparent;margin:0;padding:0" class="alert alert-danger">{{ $message }}</div>
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input class="form-control tab-child" type="date" name="date_from" value="{{$item->date_from}}" placeholder="من">
                            @error('date_from')
                                <div style="background:transparent;margin:0;padding:0" class="alert alert-danger">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input class="form-control tab-child" type="date" name="date_to" value="{{$item->date_to}}" placeholder="الى">
                            @error('date_to')
                                <div style="background:transparent;margin:0;padding:0" class="alert alert-danger">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <textarea class="form-control tab-child " type="text" name="ar[description]"  >{{$item->translate('ar')->description}}</textarea>
                        ...
                        <input type="file" name="img" id="file-3" class="inputfile" data-multiple-caption="{count} files selected" >
                            <label for="file-3" class="if-style-1 if-primary">
                                ...
                                <img style="width: 100%;height: 100%;border-radius: 50%;" src="{{asset('images/offer').'/'.$item->image}}">
                                </div><!-- icon-wrapper -->
