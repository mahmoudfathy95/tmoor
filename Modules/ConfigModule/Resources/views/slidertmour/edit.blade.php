        ...
        <div class="col-md-12">
            ...
            <div class="card card-primary">
                <div class="card-header">
                    <h3 class="card-title">تعديل سليدر</h3>
                </div>
                <!-- /.card-header -->
                <!-- form start -->
                <form id="create1" role="form" action="{{route('sliderTmour.update',$item->id)}}" method="post" enctype="multipart/form-data">
                    @csrf
                    @method('put')
                    <div class="card-body">
                    <div class="row">
                     <div class="form-group col-md-6">
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <select name="cat_type" class="form-control select2" data-placeholder="Choose Browser" >
                                ...
                                <option {{$item->cat_type==1?'selected':''}} value="1">اقسام</option>
                                <option {{$item->cat_type==2?'selected':''}} value="2">منتجات</option>
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <select name="product" class="form-control select2" data-placeholder="Choose Browser" >
                             @foreach($products as $key)
                                <option {{$item->cat_type==2&&$key->id==$item->cat_id?'selected':''}} value="{{$key->id}}">{{$key->name}}</option>
                              @endforeach
                            </select>
                            @error('cat_type')
                                <div style="background:transparent;margin:0;padding:0" class="alert alert-danger">{{ $message }}</div>
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <select name="category" class="form-control select2" data-placeholder="Choose Browser" >
                             @foreach($categories as $key)
                                <option {{$item->cat_type==1&&$key->id==$item->cat_id?'selected':''}} value="{{$key->id}}">{{$key->name}}</option>
                            @endforeach
                            </select>
                            @error('cat_type')
                                <div style="background:transparent;margin:0;padding:0" class="alert alert-danger">{{ $message }}</div>
